...
#define rw_descriptor 0x00300000
...
int kylin_cr_init(){
	...
	// ref U-Boot64/drivers/mmc/rtkemmc.c to speed up emmc
...
int rtk_eMMC_init( void )
{
    ...
    {
		prints("rtk_emmc : detect chip rev. >= B\n");
...
void make_ip_des(unsigned char *dma_addr, UINT32 dma_length){
	...
	UINT32* des_base = (UINT32*)rw_descriptor;
	UINT32  blk_cnt;
	UINT32  blk_cnt2;
