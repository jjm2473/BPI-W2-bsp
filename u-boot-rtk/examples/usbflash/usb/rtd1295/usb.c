...
void rtk_usb_clock_init(void)
{
	...
	TRACEF("Realtek-usb: init start soft_reset1=%x, soft_reset2=%x, clock_enable1=%x\n",
	...
	mdelay(200);
	TRACEF("Realtek-usb: Enable PLL soft_reset1=%x, soft_reset2=%x, clock_enable1=%x\n",
		(uint32_t)(readl((volatile u32*)reset1)),
		(uint32_t)(readl((volatile u32*)reset2)),
		(uint32_t)(readl((volatile u32*)clk_en1)));
	...
	TRACEF("Realtek-usb: trigger bit4 CLK_EN_USB soft_reset1=%x, soft_reset2=%x, clock_enable1=%x\n",
		(uint32_t)(readl((volatile u32*)reset1)),
		(uint32_t)(readl((volatile u32*)reset2)),
		(uint32_t)(readl((volatile u32*)clk_en1)));
...
do {  \
	if (get_chip_id() == RTK1296_CPU_ID) { \
		TRACEF("RTD1296 SET %s #%d enable=%d\n", \
...
void rtk_usb_power_on(void)
{
	...
	check = readl((volatile u32*)USB_TYPEC_STS);
		TRACEF("Realtek-usb: type_c cc status=0x%x\n", check);
	if ((check & 0x7) != 0x0) {
		TRACEF("Realtek-usb: cc1 detect type_c have power\n");
		goto out;
	} else if ((check & 0x38) != 0x0) {
		TRACEF("Realtek-usb: cc2 detect type_c have power\n");
		goto out;
	}
	...
	check = readl((volatile u32*)USB_TYPEC_STS);
	if ((check & 0x7) != 0x7) {
		TRACEF("Realtek-usb: cc1 detect\n");
		type_c_have_device = 1;
	} else if ((check & 0x38) != 0x38) {
		TRACEF("Realtek-usb: cc2 detect\n");
		type_c_have_device = 1;
	}
	...
	if (type_c_have_device) {
		...
		TRACEF("Realtek-usb: Type C port no device, turn off port 0 power\n");
		USB_PORT0_POWER(0);
	}

out:
	//Usb2 5V
	TRACEF("Realtek-usb: Turn on port 1 and 2 power\n");
	USB_PORT1_POWER(1);
	USB_PORT2_POWER(1);

	if (get_chip_id() == RTK1296_CPU_ID) {
		TRACEF("Realtek-usb: Turn on 1296 usb port3 power\n");
		USB_PORT3_POWER(1);
	}
}
#endif
